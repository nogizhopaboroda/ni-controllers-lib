...
function LCDDigit(config, outPacket, invalidateOutput) {
  var char = ' ';
  var dotBrightness = 0;
  var brightness = 1;
  ...
  var updateOutputPacket = function() {
    ...
    outPacket[config.a] = (segmentBits & 0x01) ? value : 0;
    outPacket[config.b] = (segmentBits & 0x02) ? value : 0;
    outPacket[config.c] = (segmentBits & 0x04) ? value : 0;
    outPacket[config.d] = (segmentBits & 0x08) ? value : 0;
    outPacket[config.e] = (segmentBits & 0x10) ? value : 0;
    outPacket[config.f] = (segmentBits & 0x20) ? value : 0;
    outPacket[config.g] = (segmentBits & 0x40) ? value : 0;
    outPacket[config.dp] = Math.floor(dotBrightness*127);
    invalidateOutput();
  };
