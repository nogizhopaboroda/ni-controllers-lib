...
const TouchStrip = require('./components/input/touch_strip');
  ...
  constructor(config) {
    ...
    const processInputBlock = (name, iconf) => {
      ...
      if (iconf.touchStrips) {
        for (var key in iconf.touchStrips) {
          const ts = this.touchStrips[key] =
            new TouchStrip(key, iconf.touchStrips[key], this);
          widgets.push(ts);
        }
      }
    ...
    const processOutputBlock = (name, oconf) => {
      ...
      if (oconf.indexed_leds) {
        const indexed_led_mapping = this.indexed_led_mapping =
          oconf.indexed_led_mapping;
